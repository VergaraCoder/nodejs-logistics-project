...
const {addData, readData, createOtherEndpoints, returnDataFind, writeData, returnIndexFind, verifyWerehouse}=require("../functions/functions");
...
router.get("/warehouses/:id",verifyParams,async(req,res)=>{
    try{
        const data=await readData();
        const dataWarehouse=data.warehouses.find(item=>{return item.id==parseInt(req.params.id)});
    
        if(!dataWarehouse) throw new Error("THE WAREHOUSE NOT EXIST");
        else {
            dataWarehouse.links=createOtherEndpoints("warehouses",parseInt(req.params.id));
            res.status(200).json({
...
router.put("/warehouses/:id",verifyParams,async(req,res)=>{
    try{
...
router.delete("/warehouses/:id",verifyParams,async(req,res)=>{
    try{
...
router.get("/shipments/:id",verifyParams,async(req,res)=>{
    try{
        const id=parseInt(req.params.id);
...
router.put("/shipments/:id",verifyParams,async(req,res)=>{
    try{
        const id=parseInt(req.params.id);
        const data=await readData();
        const index=returnIndexFind(data.shipments,id);
...
router.delete("/shipments/:id",verifyParams,async(req,res)=>{
    try{
        const id=parseInt(req.params.id);
        const data=await readData();
        const index=returnIndexFind(data.shipments,id);
...
router.get("/drivers/:id",verifyParams,async(req,res)=>{
    try{
        const id=parseInt(req.params.id);
        const data=await readData();
